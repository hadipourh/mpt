...
import random
from multiprocessing import Pool, current_process, Event
...
global distinguisher_found
distinguisher_found = multiprocessing.Event()
...
class Warp:
    ...
    def check_cube(self, active_indices=list(range(127)), target_output_bits = range(128)):
        ...
        return tuple(balanced_bits)
    
def chunk_search_for_distinguisher(nr, chunk, solver_name="minisat22"):
    warp = Warp(nrounds=nr,
    ...
    cnt= 0
    while cnt < len(chunk) and not distinguisher_found.is_set():
        active_nibbles  = chunk[cnt]
        active_bits = []
        ...
        if balanced_bits != ():
            ...
            distinguisher_found.set()
            print(f"Integral distinguisher found in thread {pid}")
            break
        cnt += 1
    return (distinguisher_flag, active_nibbles, balanced_bits)

def chunk_search_for_the_best_distinguisher(nr, chunk, solver_name, best_active_nibbles, best_balanced_bits):
    warp = Warp(nrounds=nr,
                solver_name=solver_name)
    warp.generate_sat_model()
    balanced_bits = []
    ...
    for active_nibbles in chunk:
        ...
        for nibble in active_nibbles:
            ...
            if len(balanced_bits) > len(best_balanced_bits):
                ...
                distinguisher_flag = True
    return (distinguisher_flag, best_active_nibbles, best_balanced_bits)

if __name__ == '__main__':
    NUMBER_OF_ROUNDS = 15
    NUM_OF_THREADS = 70
    solver_name = "minisat22"
    number_of_active_nibbles = 6
    ...
    while not distinguisher_flag:
        # Precompute the possible combinations of active nibbles
        possible_active_patterns = list(combinations(list(range(32)), number_of_active_nibbles))
        num_of_possible_patterns = len(possible_active_patterns)
        size_of_chunks = num_of_possible_patterns // NUM_OF_THREADS
        arguments = [(NUMBER_OF_ROUNDS, possible_active_patterns[size_of_chunks*i:size_of_chunks*(i + 1)], solver_name) \
                    for i in range(NUM_OF_THREADS)]
        if num_of_possible_patterns % NUM_OF_THREADS != 0:
            remaining_cases = num_of_possible_patterns % NUM_OF_THREADS
            arguments += [(NUMBER_OF_ROUNDS, possible_active_patterns[size_of_chunks*NUM_OF_THREADS:(size_of_chunks*NUM_OF_THREADS + remaining_cases)], solver_name)]
        del possible_active_patterns
        
        number_of_computations_per_thread = log2(size_of_chunks)
        print(f"Looking for a distinguisher with {number_of_active_nibbles} active nibbles ...")
        print("Number of cube searches per thread: 2^(%0.02f)" % number_of_computations_per_thread)
    
        distinguisher_found.clear()
        with Pool() as pool:
            results = pool.starmap(chunk_search_for_distinguisher, arguments)
        del arguments
        for thread_result in results:
            (distinguisher_flag, active_nibbles, balanced_bits) = thread_result
            if distinguisher_flag == True:
                break
        number_of_active_nibbles += 1
    elapsed_time = time.time() - start_time
    print("Total time to find an integral distinguisher: %0.02f seconds" % elapsed_time)
    start_time = time.time()
    ###################################################################################################
    ###################################################################################################
    number_of_active_nibbles -= 1
    ...
    num_of_possible_patterns = len(possible_active_patterns)
    size_of_chunks = num_of_possible_patterns // NUM_OF_THREADS
    arguments = [(NUMBER_OF_ROUNDS,
                  possible_active_patterns[size_of_chunks*i:size_of_chunks*(i + 1)],
                  solver_name,
                  best_active_nibbles,
                  best_balanced_bits)
                  for i in range(NUM_OF_THREADS)]
    if num_of_possible_patterns % NUM_OF_THREADS != 0:
        remaining_cases = num_of_possible_patterns % NUM_OF_THREADS
        arguments += [(NUMBER_OF_ROUNDS,
                       possible_active_patterns[size_of_chunks*NUM_OF_THREADS:(size_of_chunks*NUM_OF_THREADS + remaining_cases)],
                       solver_name,
                       best_active_nibbles,
                       best_balanced_bits)
                       for i in range(NUM_OF_THREADS)]
    del possible_active_patterns
    with Pool() as pool:
        results = pool.starmap(chunk_search_for_the_best_distinguisher, arguments)
    del arguments
    for thread_result in results:
        distinguisher_flag, active_nibbles, balanced_bits = thread_result
        if len(best_balanced_bits) < len(balanced_bits):
            best_active_nibbles = active_nibbles
            best_balanced_bits = balanced_bits
    result_file_name = f"result_parallel_{NUMBER_OF_ROUNDS}_nr.txt"
    with open(result_file_name, "w") as outputfile:
    ...
    elapsed_time = time.time() - start_time
    print("Total time to find the best integral distinguisher: %0.02f seconds" % elapsed_time)
